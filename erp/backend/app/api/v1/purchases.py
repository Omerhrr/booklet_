...
async def get_bill(
    ...
    purchase_service = PurchaseService(db)
    bill = purchase_service.get_by_id(bill_id, current_user.business_id)
    if not bill:
...
async def list_debit_notes(
    ...
    return dn_service.get_by_business(current_user.business_id)
...
async def get_debit_note(
    ...
    dn_service = DebitNoteService(db)
    dn = dn_service.get_by_id(debit_note_id, current_user.business_id)
    if not dn:
